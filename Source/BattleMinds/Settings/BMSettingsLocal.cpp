...
#include "Audio/BMAudioSettings.h"
#include "Development/BMPlatformEmulationSettings.h"
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("BattleMinds.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("BattleMinds.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("BattleMinds.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("BattleMinds.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("BattleMinds.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("BattleMinds.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("BattleMinds.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("BattleMinds.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("BattleMinds.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("BattleMinds.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
namespace BMSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = UBMSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UBMSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UBMSettingsLocal::UBMSettingsLocal()
{
...
void UBMSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
...
void UBMSettingsLocal::LoadSettings(bool bForceReload)
{
	...
	BMSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);

	DesiredMobileFrameRateLimit = MobileFrameRateLimit;
	ClampMobileQuality();
}

void UBMSettingsLocal::ResetToCurrentSettings()
{
...
void UBMSettingsLocal::BeginDestroy()
{
...
UBMSettingsLocal* UBMSettingsLocal::Get()
{
	return GEngine ? CastChecked<UBMSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UBMSettingsLocal::ConfirmVideoMode()
{
...
float UBMSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UBMPlatformSpecificRenderingSettings* PlatformSettings = UBMPlatformSpecificRenderingSettings::Get();
...
int32 UBMSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return BMSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UBMSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UBMSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UBMSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UBMSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UBMSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UBMSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UBMSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
float UBMSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UBMSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UBMSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UBMSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UBMSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UBMSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UBMSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UBMSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UBMSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UBMSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UBMSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UBMSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UBMSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UBMSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UBMSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UBMSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return BMSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UBMSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return BMSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UBMSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UBMSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UBMPlatformSpecificRenderingSettings* PlatformSettings = UBMPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == EFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return BMSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UBMSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UBMPlatformSpecificRenderingSettings* PlatformSettings = UBMPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EFramePacingMode::MobileStyle)
	{
...
void UBMSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UBMSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UBMPlatformSpecificRenderingSettings* PlatformSettings = UBMPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = BMSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UBMSettingsLocal::ClampMobileQuality()
{
	const UBMPlatformSpecificRenderingSettings* PlatformSettings = UBMPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EFramePacingMode::MobileStyle)
	{
...
void UBMSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = BMSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UBMSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = BMSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = BMSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UBMSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UBMSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UBMSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UBMSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UBMSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = BMSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool UBMSettingsLocal::CanRunAutoBenchmark() const
{
	const UBMPlatformSpecificRenderingSettings* PlatformSettings = UBMPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UBMSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UBMSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UBMSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float UBMSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UBMSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UBMSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UBMSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UBMSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UBMSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UBMSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UBMSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UBMSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UBMSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UBMSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void UBMSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UBMSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UBMSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UBMSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UBMSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == EFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = BMSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UBMSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UBMSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UBMSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const UBMAudioSettings* BMAudioSettings = GetDefault<UBMAudioSettings>())
			{
				...
				if (UObject* ObjPath = BMAudioSettings->OverallVolumeControlBus.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from BattleMinds Audio Settings."));
					}
				}

				if (UObject* ObjPath = BMAudioSettings->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from BattleMinds Audio Settings."));
					}
				}

				if (UObject* ObjPath = BMAudioSettings->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from BattleMinds Audio Settings."));
					}
				}

				if (UObject* ObjPath = BMAudioSettings->DialogueVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Dialogue Control Bus reference missing from BattleMinds Audio Settings."));
					}
				}

				if (UObject* ObjPath = BMAudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from BattleMinds Audio Settings."));
					}
				}

				if (UObject* ObjPath = BMAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from BattleMinds Audio Settings."));
					}
				}
			}
		}
...
void UBMSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UBMSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const UBMPlatformSpecificRenderingSettings* PlatformSettings = UBMPlatformSpecificRenderingSettings::Get();
	const TArray<FBMQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FBMQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UBMPlatformEmulationSettings* Settings = GetDefault<UBMPlatformEmulationSettings>();
	...
	{
	case EFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case EFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case EFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UBMSettingsLocal::UpdateConsoleFramePacing()
{
...
void UBMSettingsLocal::UpdateDesktopFramePacing()
{
...
void UBMSettingsLocal::UpdateMobileFramePacing()
{
...
void UBMSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
