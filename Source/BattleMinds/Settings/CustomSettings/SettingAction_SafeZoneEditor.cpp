...
#include "SettingAction_SafeZoneEditor.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SettingAction_SafeZoneEditor)

class UGameSetting;

#define LOCTEXT_NAMESPACE "BMSettings"

USettingAction_SafeZoneEditor::USettingAction_SafeZoneEditor()
{
	SafeZoneValueSetting = NewObject<USettingValueScalarDynamic_SafeZoneValue>();
...
TArray<UGameSetting*> USettingAction_SafeZoneEditor::GetChildSettings()
{
	return { SafeZoneValueSetting };
}

void USettingValueScalarDynamic_SafeZoneValue::ResetToDefault()
{
	Super::ResetToDefault();
	SSafeZone::SetGlobalSafeZoneScale(TOptional<float>(DefaultValue.Get(0.0f)));
}

void USettingValueScalarDynamic_SafeZoneValue::RestoreToInitial()
{
