...
#include "SettingKeyboardInput.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SettingKeyboardInput)

class ULocalPlayer;

#define LOCTEXT_NAMESPACE "BMSettings"

namespace BattleMinds::ErrorMessages
{
	static const FText UnknownMappingName = LOCTEXT("BMErrors_UnknownMappingName", "Unknown Mapping");
}


USettingKeyboardInput::USettingKeyboardInput()
{
	bReportAnalytics = false;
}

FText USettingKeyboardInput::GetSettingDisplayName() const
{
	...
	return BattleMinds::ErrorMessages::UnknownMappingName;
}

FText USettingKeyboardInput::GetSettingDisplayCategory() const
{
	if (const FKeyMappingRow* Row = FindKeyMappingRow())
	{
		if (Row->HasAnyMappings())
...
const FKeyMappingRow* USettingKeyboardInput::FindKeyMappingRow() const
{
...
UEnhancedPlayerMappableKeyProfile* USettingKeyboardInput::FindMappableKeyProfile() const
{
...
UEnhancedInputUserSettings* USettingKeyboardInput::GetUserSettings() const
{
	if(UBMLocalPlayer* BMLocalPlayer = Cast<UBMLocalPlayer>(LocalPlayer))
	{
		// Map the key to the player key profile
		if (UEnhancedInputLocalPlayerSubsystem* System = BMLocalPlayer->GetSubsystem<UEnhancedInputLocalPlayerSubsystem>())
		{
...
void USettingKeyboardInput::OnInitialized()
{
...
void USettingKeyboardInput::InitializeInputData(const UEnhancedPlayerMappableKeyProfile* KeyProfile, const FKeyMappingRow& MappingData, const FPlayerMappableKeyQueryOptions& InQueryOptions)
{
	check(KeyProfile);
...
FText USettingKeyboardInput::GetKeyTextFromSlot(const EPlayerMappableKeySlot InSlot) const
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
...
void USettingKeyboardInput::ResetToDefault()
{	
	if (UEnhancedInputUserSettings* Settings = GetUserSettings())
	{
...
void USettingKeyboardInput::StoreInitial()
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
		if(const FKeyMappingRow* Row = FindKeyMappingRow())
...
void USettingKeyboardInput::RestoreToInitial()
{
...
bool USettingKeyboardInput::ChangeBinding(int32 InKeyBindSlot, FKey NewKey)
{
...
void USettingKeyboardInput::GetAllMappedActionsFromKey(int32 InKeyBindSlot, FKey Key, TArray<FName>& OutActionNames) const
{	
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{		
...
bool USettingKeyboardInput::IsMappingCustomized() const
{
	bool bResult = false;
