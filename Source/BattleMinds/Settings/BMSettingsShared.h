...
#include "BMSettingsShared.generated.h"
...
/**
 * UBMSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class BATTLEMINDS_API UBMSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UBMSettingsShared, FOnSettingChangedEvent, UBMSettingsShared* Settings);
...
public:
	...
	static UBMSettingsShared* CreateTemporarySettings(const UBMLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static UBMSettingsShared* LoadOrCreateSettings(const UBMLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UBMSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UBMLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
